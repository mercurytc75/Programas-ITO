...
public class Fundidora {
    ...
    public static final int COSTO_ENFRIAR = 100;
     ...
     */
    public synchronized void procesarMinerales(
        int idRobot,
        ...
        try {
            ...
            if (sobrecalentada) {
                System.out.printf(
                    "[FUNDIDORA] ¡SOBRECALENTADA! Robot #%d debe esperar...%n",
                    idRobot
                );
                return;
            }
            ...
            System.out.printf(
                "[FUNDIDORA] Iniciando procesamiento para Robot #%d%n",
                idRobot
            );
            ...
            // Aumentar calor
            nivelCalor = Math.min(
                UMBRAL_SOBRECALENTAMIENTO,
                nivelCalor + velocidadSubida
            );

            System.out.printf(
                "[FUNDIDORA] Procesamiento completado. %d minerales refinados. Total: %d | Calor: %d%%%n",
                cantidadProcesada,
                mineralRefinado,
                nivelCalor
            );
        ...
        new Thread(() -> {
            try {
                ...
                synchronized (Fundidora.this) {
                    ...
                    System.out.println(
                        "[FUNDIDORA] Recuperada del sobrecalentamiento"
                    );
                }
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
    ...
    public synchronized boolean enfriar(int dineroDisponible) {
        ...
        if (sobrecalentada && nivelCalor < 50) {
            sobrecalentada = false;
            System.out.println(
                "[FUNDIDORA] Sobrecalentamiento cancelado por enfriamiento manual"
            );
        }
    ...
    public synchronized void setNivelCalor(int calor) {
        this.nivelCalor = Math.max(
            0,
            Math.min(UMBRAL_SOBRECALENTAMIENTO, calor)
        );
    }
    ...
    public synchronized void reducirVelocidadCalor() {
        ...
        System.out.printf(
            "[FUNDIDORA] Velocidad de calor optimizada: +%d%% por lote, -%d%% por segundo%n",
            velocidadSubida,
            velocidadBajada
        );
    }
