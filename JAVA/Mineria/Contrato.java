...
public class Contrato {
    ...
    public Contrato(int id, String tipoMineral, int cantidadRequerida, 
                    int tiempoSegundos, int recompensa, int penalizacion) {
        this.id = id;
    ...
    public int getTiempoRestante() {
        ...
        long tiempoTranscurrido = (System.currentTimeMillis() - tiempoCreacion) / 1000;
    ...
    public synchronized boolean agregarMinerales(int cantidadARestar) {
        ...
        if (cantidadEntregada >= cantidadRequerida) {
            completado = true;
            System.out.printf("✅ [CONTRATO #%d] CUMPLIDO: %s %d/%d | Recompensa: $%d%n", 
                    id, tipoMineral, cantidadEntregada, cantidadRequerida, recompensa);
            return true;
        }
        
        System.out.printf("📋 [CONTRATO #%d] Progreso: %s %d/%d%n", 
                id, tipoMineral, cantidadEntregada, cantidadRequerida);
        return false;
    }
    ...
    public String getDescripcion() {
        return String.format("⚠️  %s x%d en %ds → Recomp: $%d", 
                tipoMineral, cantidadRequerida, tiempoLimiteSegundos, recompensa);
    }
    ...
    public String getProgressBar() {
        ...
        int barLength = 10;
        int filled = (int) (barLength * (1.0 - (tiempoRestante / (double) tiempoLimiteSegundos)));
    ...
    public void setAceptado(boolean aceptado) {
        ...
        if (aceptado) {
            System.out.printf("🤝 [CONTRATO #%d] ACEPTADO: %s x%d%n", 
                    id, tipoMineral, cantidadRequerida);
        }
    }
