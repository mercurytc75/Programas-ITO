...
public class GestorEventos {
    ...
    private void bucleEventos() {
        ...
        while (activo) {
            try {
                ...
                int tiempoEspera =
                    INTERVALO_EVENTOS_MIN +
                    random.nextInt(
                        INTERVALO_EVENTOS_MAX - INTERVALO_EVENTOS_MIN
                    );
                ...
                TipoEvento evento = TipoEvento.values()[random.nextInt(
                    TipoEvento.values().length
                )];
    ...
    private void eventoFalloElectrico() {
        ...
        fundidora.setNivelCalor(100);

        new Thread(() -> {
            try {
                Thread.sleep(10000);
                synchronized (fundidora) {
                    // Cool down partially after the event ends
                    if (fundidora.getNivelCalor() >= 80) {
                        fundidora.setNivelCalor(60);
                    }
                }
                System.out.println(
                    "[EVENTO] El fallo eléctrico terminó — fundidora estabilizándose"
                );
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
    ...
    private void eventoDerrumbe() {
        ...
        System.out.printf(
            "¡¡¡Derrumbe en Zona %s!!! Bloqueada por 20 segundos%n",
    ...
    private void eventoRobotDañado() {
        ...
        int energiaActual = robotAfectado.getEnergia();
        int nuevaEnergia = Math.max(0, energiaActual - 50);
        robotAfectado.setEnergia(nuevaEnergia);
    ...
    private void eventoVetaRica() {
        ...
        System.out.printf(
            "¡¡¡Veta rica descubierta en Zona %s!!! Los minerales se duplican por 15 segundos%n",
        ...
        new Thread(() -> {
            try {
                Thread.sleep(15000);
                System.out.printf(
                    "[EVENTO] La veta rica en Zona %s se agotó%n",
                    zonaAfortunada.getNombre()
                );
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
