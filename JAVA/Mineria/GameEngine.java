...
public class GameEngine {
    ...
    public synchronized boolean crearRobot(int id, int nivel) {
        ...
        if (dinero < costo) {
            System.out.printf("[ALERTA] Dinero insuficiente para comprar Robot Nivel %d (costo: $%d)%n", nivel, costo);
            return false;
        }
        dinero -= costo;
        ...
        robots.add(robot);
        robot.start();
        System.out.printf("[OK] Robot #%d (Nivel %d) comprado y asignado a Zona %s%n", id, nivel, zonaInicial.getNombre());
        return true;
    }
    ...
    public void iniciar() {
        ...
        System.out.println("\n" + "=".repeat(70));
        System.out.println("INICIANDO MINETRUSH - JUEGO DE MINERIA EN TIEMPO REAL");
        System.out.println("=".repeat(70));
        System.out.printf("Dinero inicial: $%d%n", dinero);
        System.out.printf("Robots activos: %d%n", robots.size());
        System.out.printf("Rondas: %d (2 min cada una = 20 min totales)%n", rondasTotales);
        System.out.println("Zonas disponibles: Zona A (Cobre/Hierro), Zona B (Plata/Oro), Zona C (Diamante)");
        System.out.println("=".repeat(70) + "\n");
        ...
        new Thread(this::manejarRondas).start();
    ...
    private void manejarRondas() {
        for (int r = 1; r <= rondasTotales; r++) {
            ...
            System.out.printf("%n[NOTICIA] RONDA %d/%d INICIADA%n", r, rondasTotales);
            System.out.printf("Dinero: $%d | Robots activos: %d%n", dinero, contarRobotsActivos());
            try {
            ...
            if (dinero <= 0 || (contarRobotsActivos() == 0 && dinero < PRECIO_ROBOT_NIVEL_1)) {
                finalizarJuego("DERROTA", "Te quedaste sin dinero y sin robots para continuar");
                return;
            }
        }
        if (dinero > 0) {
            finalizarJuego("VICTORIA", "Completaste todas las rondas con ganancia");
        } else {
    ...
    public synchronized boolean recargarRobot(int robotId) {
        Robot robot = robots.stream().filter(r -> r.getRobotId() == robotId).findFirst().orElse(null);
        if (robot == null) {
            System.out.println("[ALERTA] Robot no encontrado");
            return false;
        }
        if (dinero < PRECIO_RECARGA) {
            System.out.printf("[ALERTA] Dinero insuficiente para recargar Robot #%d (costo: $%d)%n", robotId, PRECIO_RECARGA);
            return false;
        }
        dinero -= PRECIO_RECARGA;
        robot.recargarEnergia(100);
        System.out.printf("[OK] Robot #%d recargado. Dinero restante: $%d%n", robotId, dinero);
        return true;
    }
    ...
    public synchronized boolean repararRobot(int robotId) {
        ...
        if (dinero < PRECIO_REPARACION) {
            System.out.printf("[ALERTA] Dinero insuficiente para reparar Robot #%d (costo: $%d)%n", robotId, PRECIO_REPARACION);
            return false;
        }
        dinero -= PRECIO_REPARACION;
        robot.repararRobot();
        System.out.printf("[OK] Robot #%d reparado. Dinero restante: $%d%n", robotId, dinero);
        return true;
    }
    
    public synchronized void asignarRobotAZona(int robotId, String nombreZona) {
        Robot robot = robots.stream().filter(r -> r.getRobotId() == robotId).findFirst().orElse(null);
        ZonaMina zona = zonas.stream().filter(z -> z.getNombre().equals(nombreZona)).findFirst().orElse(null);
        if (robot != null && zona != null) {
            robot.setZona(zona);
            System.out.printf("[OK] Robot #%d asignado a Zona %s%n", robotId, nombreZona);
        }
    }
    ...
    public synchronized void enfriarFundidora() {
        ...
        if (fundidora.enfriar(dinero)) {
            dinero -= 100;  // Aquí se descuenta el costo
            System.out.printf("[OK] Fundidora enfriada. Dinero restante: $%d%n", dinero);
        } else {
            System.out.println("[ALERTA] Dinero insuficiente para enfriar fundidora (costo: $100)");
        }
    }
    ...
    private void mostrarEstadisticas() {
        while (juegoActivo) {
            try {
                ...
                long tiempoRestante = Math.max(0, tiempoRondaMs - tiempoTranscurrido);
                int segundosRestantes = (int) (tiempoRestante / 1000);
                System.out.println("\n========== ESTADO DEL JUEGO ==========");
                System.out.printf("[DINERO] $%-6d | [RONDA] %d/%d | [TIEMPO] %02d:%02d seg%n", dinero, rondaActual, rondasTotales, segundosRestantes / 60, segundosRestantes % 60);
                System.out.printf("[FUNDIDORA] %d refinados | Calor: %d%% | [BODEGA] %d/%d minerales%n", 
                        fundidora.getMineralRefinado(), fundidora.getNivelCalor(), 
                        bodega.getCantidadAlmacenada(), bodega.getCapacidadMaxima());
                System.out.println("--- ROBOTS ACTIVOS ---");
                for (Robot robot : robots) {
                    System.out.printf("[Robot #%-2d] Nivel: %d | Energia: %-3d%% | Zona: %s | Estado: %s%n", robot.getRobotId(), robot.getNivel(), robot.getEnergia(), robot.getZonaAsignada(), robot.getEstado());
                }
                System.out.println("-- Zonas disponibles --");
                for (ZonaMina zona : zonas) {
                    String estado = zona.isBloqueada() ? "BLOQUEADA" : "Disponible";
                    System.out.printf("Zona %s: %s - Reserva: %d/%d (%d%%)%n", zona.getNombre(), estado, zona.getReserva(), zona.getReservaMaxima(), zona.getPorcentajeReserva());
                }
                
                // FASE 3: Mostrar contratos activos
                List<Contrato> contratos = gestorContratos.getContratosActivos();
                if (!contratos.isEmpty()) {
                    System.out.println("-- Contratos activos --");
                    for (Contrato c : contratos) {
                        String aceptado = c.isAceptado() ? "ACEPTADO" : "PENDIENTE";
                        System.out.printf("[Contrato #%d] %s - %s %s%n", 
                                c.getId(), aceptado, c.getProgressBar(), c.getTipoMineral());
                    }
                }
    ...
    private int contarRobotsActivos() {
        return (int) robots.stream().filter(r -> r.isActivo() && r.getEstado() == Robot.Estado.ACTIVO).count();
    }
    ...
    private synchronized void finalizarJuego(String resultado, String motivo) {
        ...
        System.out.printf("Ronda completada: %d/%d%n", rondaActual, rondasTotales);
        System.out.printf("Robots activos: %d%n", contarRobotsActivos());
        System.out.printf("Minerales totales refinados: %d%n", fundidora.getMineralRefinado());
        System.out.printf("Contratos completados: %d | Total ganado: $%d | Total perdido: -$%d%n",
    ...
    public synchronized boolean crearRobotPlus(int id, int nivel) {
        ...
        robots.add(robot);
        robot.start();
        System.out.printf("[OK] Robot #%d (Nivel %d) agregado a la flota%n", id, nivel);
        return true;
    }
    ...
    public synchronized int getDinero() { return dinero; }
    public int getRondaActual() { return rondaActual; }
    public int getRondasTotales() { return rondasTotales; }
    public List<Robot> getRobots() { return new ArrayList<>(robots); }
    public List<ZonaMina> getZonas() { return new ArrayList<>(zonas); }
    public Fundidora getFundidora() { return fundidora; }
