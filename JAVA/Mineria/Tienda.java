...
public class Tienda {
    ...
    public synchronized boolean comprarRobot(int nivel) {
        if (nivel < 1 || nivel > 3) {
            System.out.println("❌ [TIENDA] Nivel de robot inválido. Disponibles: 1, 2, 3");
            return false;
        }
        ...
        if (dineroDisponible < costo) {
            ...
            System.out.printf("❌ [TIENDA] Dinero insuficiente. Tienes $%d, falta $%d%n", 
                    dineroDisponible, falta);
            return false;
        }
        ...
        int idRobot = totalRobotComprados + 1;
        gameEngine.crearRobotPlus(idRobot, nivel);
        
        totalRobotComprados++;
        totalDineroGastado += costo;
        
        String descripcion = String.format("Robot Nivel %d - $%d", nivel, costo);
        historialCompras.add(new ItemCompra("Robot", descripcion, costo));
        
        System.out.printf("✅ [TIENDA] Robot #%d (Nivel %d) comprado por $%d%n", 
                idRobot, nivel, costo);
    ...
    public synchronized boolean mejorarFundidora() {
        ...
        if (dineroDisponible < costo) {
            int falta = costo - dineroDisponible;
            System.out.printf("❌ [TIENDA] Dinero insuficiente para mejora de fundidora. Falta $%d%n", falta);
            return false;
        }
        ...
        String descripcion = String.format("Mejora Fundidora Nivel %d - $%d", nivelFundidora, costo);
        historialCompras.add(new ItemCompra("Mejora Fundidora", descripcion, costo));
        
        System.out.printf("✅ [TIENDA] Fundidora mejorada a Nivel %d por $%d%n", 
                nivelFundidora, costo);
    ...
    public synchronized boolean mejorarBodega() {
        ...
        if (dineroDisponible < costo) {
            int falta = costo - dineroDisponible;
            System.out.printf("❌ [TIENDA] Dinero insuficiente para mejora de bodega. Falta $%d%n", falta);
            return false;
        }
        ...
        String descripcion = String.format("Mejora Bodega Nivel %d +50 cap - $%d", nivelBodega, costo);
        historialCompras.add(new ItemCompra("Mejora Bodega", descripcion, costo));
        
        System.out.printf("✅ [TIENDA] Bodega mejorada a Nivel %d por $%d%n", 
                nivelBodega, costo);
        System.out.printf("   → Capacidad: %d unidades%n", capacidadActual + 50);
        System.out.printf("💰 Dinero restante: $%d%n", gameEngine.getDinero());
    ...
    public static String getDescripcionItem(String tipo) {
        return switch (tipo) {
            case "ROBOT_1" -> String.format("Robot Nivel 1 - $%d (lento, barato)", PRECIO_ROBOT_NIVEL_1);
            case "ROBOT_2" -> String.format("Robot Nivel 2 - $%d (normal)", PRECIO_ROBOT_NIVEL_2);
            case "ROBOT_3" -> String.format("Robot Nivel 3 - $%d (rápido, caro)", PRECIO_ROBOT_NIVEL_3);
            case "FUNDIDORA" -> String.format("Mejora Fundidora - $%d (procesa más rápido)", PRECIO_MEJORA_FUNDIDORA);
            case "BODEGA" -> String.format("Mejora Bodega - $%d (+50 capacidad)", PRECIO_MEJORA_BODEGA);
    ...
    public String obtenerMenuTienda() {
        ...
        menu.append("╚════════════════════════════════════════╝\n");
        menu.append(String.format("💰 Dinero disponible: $%d\n\n", gameEngine.getDinero()));
        
        menu.append("┌─── ROBOTS ───┐\n");
        menu.append(String.format("1. Robot Nivel 1 - $%d (Lento, barato)\n", PRECIO_ROBOT_NIVEL_1));
        menu.append(String.format("2. Robot Nivel 2 - $%d (Normal)\n", PRECIO_ROBOT_NIVEL_2));
        menu.append(String.format("3. Robot Nivel 3 - $%d (Rápido, caro)\n", PRECIO_ROBOT_NIVEL_3));
        
        menu.append("\n┌─── MEJORAS ───┐\n");
        menu.append(String.format("4. Mejorar Fundidora (Nivel %d) - $%d\n", nivelFundidora, PRECIO_MEJORA_FUNDIDORA));
        menu.append(String.format("5. Mejorar Bodega (Nivel %d) - $%d\n", nivelBodega, PRECIO_MEJORA_BODEGA));
        
        menu.append("\n┌─── INFO ───┐\n");
        menu.append(String.format("Robots comprados: %d\n", totalRobotComprados));
